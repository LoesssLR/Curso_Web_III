...
if (!docId) {
    alert("No se recibio el ID del signo a editar");
}

// static data to define the structure according to astrology
...
// functions to fill select options
...
// when elemento changes, update signos
...
// when signo changes, update related fields
...
if (docId) {
    db.collection("datosZodiaco").doc(docId).get()
        .then(function (doc) {
            if (!doc.exists) {
                alert("No se encontro el documento a editar");
                return;
            }
            const data = doc.data();
            ...
            // Guardar URL actual
            urlActual = data.url || "";
            imgPreview.src = urlActual;

            // Elemento
            selElemento.value = data.elemento || "";
            selElemento.dispatchEvent(new Event('change'));

            // Signo
            if (data.signo) {
                selSigno.value = data.signo;
                selSigno.dispatchEvent(new Event('change'));
            }

            // Astros y piedras
            if (data.astroCelestial) {
                selAstroCelestial.value = data.astroCelestial;
            }
            if (data.piedraPreciosa) {
                selPiedraPreciosa.value = data.piedraPreciosa;
            }
        ...
        .catch(function (error) {
            console.error("Error al cargar signo:", error);
            alert("Error al cargar los datos para edicion");
        });
}

// function to update the document in Firestore

function actualizarDocumento(urlFinal) {
    ...
    db.collection("datosZodiaco").doc(docId).update({
        ...
        .catch(function (error) {
            ...
            alert("Error al actualizar el signo: " + error);
        });
}
...
btnActualizar.addEventListener('click', function () {
    ...
    // if there´s no new file - we use the current URL
    if (!archivo) {
        actualizarDocumento(urlActual);
        return;
    }
    ...
    subir
        ...
        .then(urlNueva => {
            actualizarDocumento(urlNueva);
        })
        .catch(function (error) {
            console.error("Error al subir la nueva imagen:", error);
            alert("Error al subir la nueva imagen: " + error);
        });
});
